...
### 变更内容
- 排行页新增第 5 个 tab「总榜」：统计本群创建至今全部打卡（次数/千卡/分钟）累计排行，可按 3 指标排序；仅统计当前群成员；空群显示「本群暂无打卡数据」。
- 大数紧凑格式化 `utils/format.js#compactNumber`：≥1亿 → X.XX亿、≥1万 → X.XX万（两位小数、不省尾零）、<1万 → 千分位。
...
## v1.10.6 — BMI 卡编辑弹窗遮挡根治（2026-08-31，commit f95a26d）

### 变更内容
- `bmi-gauge` 新增 `dimmed` 属性：编辑资料弹窗打开时（`showEdit`）canvas 用 `hidden`（display:none，瞬时无动画）隐藏，**任何运行环境都不会再浮在弹窗上方**；关闭弹窗自动重绘。
- 保留 canvas 2d 同层渲染与 `z-index`（bmi-card:1 / modal-mask:999）双保险。
...
## v1.10.5 — 剩余优化点修复（2026-08-31，commit 100bd16）
...
### 变更内容
**云函数**
- `getFeed`：缓存 key 加入 OPENID，修复同页 15s 内他人拿到自己点赞/删除状态（跨用户泄露）。
- `getMyStats`：本月次数/卡路里累加加 `>= monthStart` 门控，修复月初（本周一在上月）时上月打卡计入本月。
- `updateCheckin`：禁止打卡跨群搬移（`groupId` 必须等于原记录群），防刷榜。
- `likeCheckin`：点赞后重读文档返回最新 `liked`/`likeCount`，缓解并发 toggle 计数漂移。

**前端**
- 首页 `loadRecent`：切群时允许 reset 覆盖、`finally` 恒清空 loading，修复最近打卡永久卡死。
- 动态页：切群时 `_feedReqId++` 并清空列表，修复旧群数据残留新群名下；评论发送中失焦不再关闭评论栏。
- 群详情页：加载态移出 `wx:if` 门，首屏不再白屏。
- 群组页：创建/加入加 `submitting` 锁，防重复提交。
- 首页「一键设置」login 补传 `weightKg`/`heightCm`。
- 我的页：日历切月加请求序号防旧月覆盖新月。
- 排行页：我的排名栏 `bottom` 加安全区。
- 打卡表单：编辑态日期范围起点放宽到记录日期。
- `constants.RANK_PERIODS` 与 mock 排行对齐 4 周期（本周/上周/本月/上月）。

### 重新部署需要进行的操作
1. 云函数（云端安装依赖）：重部署 `getFeed`、`getMyStats`、`updateCheckin`、`likeCheckin`
2. 前端：重新编译小程序

---

## v1.10.3 — 动态页加载优化（2026-08-31，commit b7279db）

### 变更内容
- 动态页**首屏默认 8 条**（`FEED_FIRST_PAGE_SIZE`），上滑触底**每页追加 5 条**（`FEED_PAGE_SIZE`）。
- 分页由「page×pageSize」改为**累计 skip**，首屏与后续页不同条数不重叠。
- `getFeed` 云函数优化：
  - **15s 内存缓存**（`cache` 包）：重复进入/切 tab 秒开，下拉刷新传 `refresh` 强制更新；
  - **用户缓存**（60s）：作者/点赞者用户信息缓存，减少重复查库；
  - **合并取图**：作者头像 + 打卡图 + 点赞者头像的一次 `getTempFileURL` 批量换取（原 3 次→1 次）。

### 重新部署需要进行的操作
- 云函数：重部署 `getFeed`（**云端安装依赖，含 cache 依赖**）
- 前端：重新编译小程序

---

## v1.10.2 — 排行页新增「上周 / 上月」+ 空态提示优化（2026-08-31，commit 6fa7cde）
