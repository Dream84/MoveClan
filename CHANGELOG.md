...
## v1.10.0 — BMI 环形指标卡（深色科技医疗风）（2026-08-31，工作区，待提交）
...
- 新增独立测试页 `pages/bmi-gauge-test`（身高/体重输入 + 采样按钮，默认 165/67）+ `tools/bmi-gauge-preview.html` 浏览器预览版。
...
### 重新部署需要进行的操作
- **仅前端**：重新编译小程序（测试页可在开发者工具中将编译模式设为 `pages/bmi-gauge-test/bmi-gauge-test` 单独打开验证）。
